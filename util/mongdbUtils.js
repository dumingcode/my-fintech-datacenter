...
module.exports = {
    async insertMany(dbName, collectionName, documents) {
        ...
        let result = await col.insertMany(documents, { ordered: false, w:'1',wtimeout:5000  })
    ...
    async insertOne(dbName, collectionName, document) {
        ...
        let db = await client.db(dbName)
        let col = await db.collection(collectionName)
        let result = await col.insertOne(document, { w:'1', wtimeout:5000 })
        await client.close(true)
        return result
    },
    async updateOne(dbName, collectionName, filter, document, upsertVal = true) {
        ...
        let db = await client.db(dbName)
        let col = await db.collection(collectionName)
        let result = await col.updateOne(filter, { $set: document }, { upsert: upsertVal,w:'1',wtimeout:5000 })
        await client.close(true)
        return result
    },
    //查询52周最低价
