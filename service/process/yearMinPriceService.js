...
module.exports = {
  async launchStockDailyDataTask () {
    ...
    const now = moment()
    const start = now.subtract(364, 'days')

    for (let i = 0; i < stockList.length; i++) {
      try {
        const yearLow = await mongdbUtils.queryMinLowPrice('stock', 'hisprice', stockList[i], Number(start.format('YYYYMMDD')))
        if (yearLow && yearLow.length > 0) {
          const data = yearLow[0]
  ...
  async saveStockLowPrice (code, xueQiuStock) {
    ...
    const x = await redisUtil.redisHSet(config.redisStoreKey.xueQiuStockSet, code, JSON.stringify(xueQiuStock))
    return x
  }
